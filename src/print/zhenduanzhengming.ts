...
const html = (data): string => {
  return `<section style="${cssStyle.printContainer}${
    cssStyle.padding10LR
            ...
            <div style="${cssStyle.title}${cssStyle.titleSize}margin:6px 0;">诊断证明书</div>
            <div style="display:flex;margin:10px 0;"><span style="width:50%;">就诊科室：${
              data.deptName
            }</span><span style="width:50%;text-align:right;padding-right:24px;">诊断日期：${
    data.registeringDate
  }</span></div>
    ...
    cssStyle.border1pxBottom
            ...
            <div style="${cssStyle.flexBetween}${cssStyle.border1pxBottom};height:120px;">
              ...
              <div style="width:85%;padding-left:5px;padding-top:5px;${
                cssStyle.borderbox
              };line-height:24px;height:120px;overflow:hidden">${data.diagDescArr
    ...
    .join('')}</div>
            ...
            <div style="${cssStyle.flexBetween}${cssStyle.border1pxBottom};height:120px;">
              ...
              <div style="width:85%;${
                cssStyle.borderbox
              };line-height:24px;padding-left:5px;overflow:hidden;height:120px;">${
            ...
            <div style="${cssStyle.flexBetween}height:32px;line-height:32px;${cssStyle.borderbox}">
              ${
                data.ukeySignValue == undefined || data.ukeySignValue == ''
                  ? `<div style="width:60%;position:relative;top:8px;padding-left:8px;">医生签字：<span>${
                      data.doctor || ''
                    }</span></div>`
                  : `<div style="width:60%;padding-left:8px;">医生签字：<img src="${data.ukeySignValue}" style="position:relative;top:8px;width:60px;height:30px;" /></div>`
              }
              <div style="width:40%;padding-left:5px;position:relative;top:8px;">打印日期：${new Date().toLocaleString()}</div>
            </div>
