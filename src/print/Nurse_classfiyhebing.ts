...
const getItem = (dataList) => {
  return dataList.map((item, index) => {
    return `<html><section style="${cssStyle.printContainer};${cssStyle.padding10LR};height:96vh;">
  ...
  <section style="display:flex;justify-content:center;font-size:16px;height:32px;line-height:32px;"> <span style="width:22%;text-align:center">日期：${
    item.printTime
      ...
      'MM-DD HH:mm'
            ...
            <div style="position:absolute;bottom:0;left:0;width:100%;display:flex;justify-content:space-between;padding:4px 20px;font-size:14px;">
              <section style="flex-grow:1; text-align:center;">第${index + 1}页，共${
      item.totals || ''
    }页</section>
            </div>
