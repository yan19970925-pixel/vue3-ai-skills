<template>
  <div class="app-container" style="padding-top: 34px">
    <content-wrap>
      <!-- 搜索工作栏 -->
      <el-row :gutter="20">
        <!--部门数据-->
...
const getList = () => {
  loading.value = true
  getUserPage(queryParams).then((response) => {
...
const handleStatusChange = (row: UserVO) => {
  ...
  message
    .confirm('确认要"' + text + '""' + row.username + '"用户吗?', '温馨提示')
    .then(async () => {
      row.status =
...
const handleResetPwd = (row: UserVO) => {
  message
    .prompt('请输入"' + row.username + '"的新密码', '温馨提示')
    .then(async ({ value }) => {
