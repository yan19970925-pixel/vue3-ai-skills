  ...
  <Dialog :title="dialogScopeTitle" v-model="dialogScopeVisible" width="800">
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button :loading="actionLoading" title="关闭" @click="dialogScopeVisible = false"
          >取 消</el-button
        >
      </div>
    </template>
...
const menuPermissionFormRef = ref() // 表单 Ref
...
const openModal = async (type: string, row: RoleApi.RoleVO) => {
  ...
  if (type === 'menu') {
    dialogScopeTitle.value = '菜单权限'
    const menuRes = await MenuApi.getSimpleMenusList()
    treeOptions.value = handleTree(menuRes)
    const role = await PermissionApi.listRoleMenus(row.id)
    if (role) {
...
const submitScope = async () => {
  if ('data' === actionScopeType.value) {
    ...
    await PermissionApi.assignRoleDataScope(data.value)
    ...
    await PermissionApi.assignRoleMenu(data.value)
  }
