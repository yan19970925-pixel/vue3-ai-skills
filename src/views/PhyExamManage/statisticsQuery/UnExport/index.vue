    ...
    <div style="width: 100%; height: 100%; overflow: auto">
      <div class="base-box mb-8px" style="background-color: #fff">
        <div class="base-cont">
          ...
          <span class="cont-span">状态:</span>
          <el-select v-model="type" placeholder="请选择" style="width: 12vw">
            <el-option
              v-for="item in types"
              :key="item.value"
              :label="item.label"
          ...
          <div class="base-cont" style="width: 28%">
            <el-button type="primary" @click="handleSaveDocument">检索</el-button>
            <el-button class="resetBtn" @click="handleSaveDocument"> 导出 </el-button>
            ...
            <el-button class="resetBtn" @click="handleSaveDocument"> 自定义报表查询 </el-button>
            <el-button class="resetBtn" @click="handleSaveDocument"> 过滤设置 </el-button>
          </div>
        </div>
      </div>
      <div
        class="mt-4px"
        style="padding: 10px; background-color: #fff; height: calc(100vh - 150px)"
      >
        <el-table
          ref="adviceTableRef"
          :data="paginatedData"
        ...
        >
          ...
          <div style="color: #3473d1; font-size: 16px; font-weight: 700">
            <span v-if="radio === 2"> 总金额: {{ 543656.43 }} 元 </span>
          </div>
          <el-pagination
            background
...
const types = ref([
  { value: 'option1', label: '常规体检' },
  { value: 'option2', label: '单位统计导出' }
])
const radio = ref(1)
...
const allColumns = [
  ...
  { prop: 'workType', label: '工种', align: 'center' },
...
const allColumns1 = [
  ...
  { prop: 'gender', label: '性别', align: 'center' },
  { prop: 'age', label: '年龄', align: 'center' },
  { prop: 'completionStatus', label: '婚姻状态', align: 'center', width: '120px' },
  { prop: 'unitName', label: '单位名称', align: 'center' },
  { prop: 'physicalExamId', label: '所属部门', align: 'center' },
  { prop: 'completionStatus', label: '当前状态', align: 'center', width: '120px' },
  { prop: 'personalCount', label: '已完成金额', align: 'center', width: '120px' },
  { prop: 'reportDate', label: '体检登记日期', align: 'center', width: '120px' },
  { prop: 'finishDate', label: '最终审核日期', align: 'center' },
  { prop: 'reportDate', label: '未完成金额', align: 'center', width: '100px' },
  { prop: 'physicalExamId', label: '打印状态', align: 'center' }
]

// 动态控制显示列
const visibleColumns = computed(() => {
  if (type.value === 'option2') return allColumns1
  return allColumns
})
...
const handleSaveDocument = () => {
  const list1 = generateMockData(['神马单位'], 10)
  const list2 = generateMockData(['星星单位'], 10)
  const list3 = generateMockData(['月亮单位'], 25)
