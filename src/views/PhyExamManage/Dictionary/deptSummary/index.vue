<template>
  <div class="base-setting">
      ...
      <el-row style="height: 100%">
        ...
        <el-col :span="20" style="height: 100%">
          <div style="width: 100%; height: calc(100%)">
            <div class="content">
              <div class="base-title">科室小结列表</div>
              <div class="base-cont">
              ...
              >
                ...
                <el-table-column label="内容" prop="peDeptConclusion" show-overflow-tooltip />
                <el-table-column
                  label="缺省值"
                  prop="isDefault"
                  width="120"
    ...
    <el-dialog v-model="dialogVisible" title="编辑小结信息" width="40%">
      <el-form :model="currentEditItem" label-width="100px">
        <el-form-item label="内容">
          <el-input v-model="currentEditItem.peDeptConclusion" />
        </el-form-item>
        <el-form-item label="缺省值">
          <el-select v-model="currentEditItem.isDefault" placeholder="请选择">
            <el-option label="是" :value="1" />
            <el-option label="否" :value="0" />
          </el-select>
        </el-form-item>
        <el-form-item label="序号">
          <el-input-number v-model="currentEditItem.serialNo" :min="0" />
        </el-form-item>
      </el-form>
...
import * as Api from '@/api/Dictionary/deptSummary/index'
...
// 科室小结-查询
const getPeDeptConclusion = async (row) => {
...
const handleSaveDocument = async () => {
  ...
  try {
    ...
    if (res) {
      ...
      getPeDeptConclusion(currentDept.value) // 可选：刷新列表
    } else {
...
const handleAddDocument = () => {
  ...
  const newItem = {
    ...
    peDeptConclusion: '',
    isDefault: 0,
    serialNo: tableList.value.length + 1 // 只在新增时生成新的序号
  }
...
const saveEdit = () => {
  if (isEditMode.value) {
    ...
    tableList.value[editIndex] = {
      ...tableList.value[editIndex], // 保留原有属性，包括序号
      peDeptConclusion: currentEditItem.value.peDeptConclusion,
      isDefault: currentEditItem.value.isDefault
      // 注意：不更新 serialNo，保持原有值
    }
  } else {
    // 新增条目：向 tableList 中添加新项
    const newItem = {
      ...currentEditItem.value,
      peDeptName: currentDept.value.peDeptName // 确保包含科室名称
    }
