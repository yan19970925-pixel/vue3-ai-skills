<template>
  <Dialog :title="modelTitle" v-model="modelVisible">
    ...
    >
      ...
      <!-- Local / FTP / SFTP -->
      <el-form-item
        v-if="parseInt(formData.storage) >= 10 && parseInt(formData.storage) <= 12"
        label="基础路径"
      ...
      <el-form-item
        v-if="parseInt(formData.storage) >= 11 && parseInt(formData.storage) <= 12"
        label="主机地址"
        prop="config.host"
      >
      ...
      <el-form-item v-if="parseInt(formData.storage) === 11" label="连接模式" prop="config.mode">
      ...
      <!-- S3 -->
      <el-form-item
        v-if="parseInt(formData.storage) === 20"
        label="节点地址"
      ...
      <el-form-item
        v-if="parseInt(formData.storage)"
        label="accessSecret"
      ...
      <!-- 通用 -->
      <el-form-item v-if="parseInt(formData.storage) === 20" label="自定义域名">
        <!-- 无需参数校验，所以去掉 prop -->
