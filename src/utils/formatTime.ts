...
/**
 * 将毫秒，转换成时间字符串。例如说，YYYY-MM-DD HH:mm:ss 或 HH:mm:ss YYYY-MM-DD
 *
 * @param ms 毫秒
 * @returns {string} 字符串
 */
export function formatMillisecondsToTime(milliseconds, type) {
  // 创建一个Date对象
  ...
  const dateTime = `${year}-${month}-${day} ${hours}:${minutes}:${seconds}`
  return dateTime // 或者 dateTime，根据你的需求选择
}
