export default class watermark {
  constructor(settings) {
    ...
    this.parentDom = settings.watermark_parent_node
      ? typeof settings.watermark_parent_node === 'string'
        ? document.querySelector(settings.watermark_parent_node)
        : settings.watermark_parent_node
      : document.body
  ...
  loadMark(settings) {
    ...
    this.defaultSettings.watermark_cols = parseInt(
      (page_width - this.defaultSettings.watermark_x) /
        (this.defaultSettings.watermark_width + this.defaultSettings.watermark_x_space)
    )
    var temp_watermark_x_space = parseInt(
      (page_width -
        this.defaultSettings.watermark_x -
        this.defaultSettings.watermark_width * this.defaultSettings.watermark_cols) /
        this.defaultSettings.watermark_cols
    )
    this.defaultSettings.watermark_x_space = temp_watermark_x_space
      ? this.defaultSettings.watermark_x_space
      : temp_watermark_x_space
    var allWatermarkWidth
    /*三种情况下会重新计算水印行数和y方向水印间隔：1、水印行数设置为0，2、水印长度大于页面长度，3、水印长度小于于页面长度*/
    this.defaultSettings.watermark_rows = parseInt(
      (page_height - this.defaultSettings.watermark_y) /
        (this.defaultSettings.watermark_height + this.defaultSettings.watermark_y_space)
    )
    var temp_watermark_y_space = parseInt(
      (page_height -
        this.defaultSettings.watermark_y -
        this.defaultSettings.watermark_height * this.defaultSettings.watermark_rows) /
        this.defaultSettings.watermark_rows
    )
    this.defaultSettings.watermark_y_space = temp_watermark_y_space
      ? this.defaultSettings.watermark_y_space
    ...
    if (watermark_parent_element) {
      allWatermarkWidth =
        this.defaultSettings.watermark_x +
        this.defaultSettings.watermark_width * this.defaultSettings.watermark_cols +
        this.defaultSettings.watermark_x_space * (this.defaultSettings.watermark_cols - 1)
      allWatermarkHeight =
        this.defaultSettings.watermark_y +
        this.defaultSettings.watermark_height * this.defaultSettings.watermark_rows +
        this.defaultSettings.watermark_y_space * (this.defaultSettings.watermark_rows - 1)
    } else {
      allWatermarkWidth =
        page_offsetLeft +
        this.defaultSettings.watermark_x +
    ...
    for (var i = 0; i < this.defaultSettings.watermark_rows; i++) {
      if (watermark_parent_element) {
        y =
          ...
          (this.defaultSettings.watermark_y_space + this.defaultSettings.watermark_height) * i
      } else {
        y =
          this.defaultSettings.watermark_y +
          (page_height - allWatermarkHeight) / 2 +
      ...
      for (var j = 0; j < this.defaultSettings.watermark_cols; j++) {
        if (watermark_parent_element) {
          x =
            ...
            (this.defaultSettings.watermark_width + this.defaultSettings.watermark_x_space) * j
        } else {
          x =
            this.defaultSettings.watermark_x +
            (page_width - allWatermarkWidth) / 2 +
