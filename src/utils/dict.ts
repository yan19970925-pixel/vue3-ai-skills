...
export const getNationByCode = (code: string) => {
  const list = dictStore.getNationDict
  const result = list.filter((item) => item.nationCode === code)
  return result.length > 0 ? result[0].nationName : ''
}

export const getBirthPlaceByCode = (code: string) => {
  const list = dictStore.getBirthPlaceDict
  const result = list.filter((item) => item.areaCode === code)
  return result.length > 0 ? result[0].areaName : ''
}
