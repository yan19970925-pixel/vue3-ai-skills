...
// 请求白名单，无须token的接口
const whiteList: string[] = ['/login', '/refresh-token', '/emrTemplate', '/viewOutpEmr']
...
service.interceptors.response.use(
  async (response: AxiosResponse<any>) => {
    ...
    if (ignoreMsgs.indexOf(msg) !== -1) {
      ...
      // 如果是病历文书界面直接返回
      if (router.currentRoute.value.path == '/emrTemplate') {
        return
      }
      if (router.currentRoute.value.path == '/viewOutpEmr') {
        return
      }
