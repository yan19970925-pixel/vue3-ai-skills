...
export function init(data = [], containerId, options) {
  const ajaxData = getData(data)
  const cHeight = 1000 + options.bottomKeys.length * 20
  const chart = ConnectedScatterplot({
    ...
    renderData: ajaxData,
    timeArr: options.timeNumber || timeNumber,
    bottomKeys: options.bottomKeys
  })
  containerId!.appendChild(chart)
}
...
function ConnectedScatterplot(options) {
  ...
  drawBottomMask(svg, viewConfig, options.bottomKeys)
  ...
  function drwaBottomLineData(svg) {
    const g = getG(svg, viewConfig)
    const bottomKeys = options.bottomKeys || BOTTOM_KEYS
    console.log('btttt', bottomKeys)
    // 绘制底部的横向线条
    g.selectAll('line')
      .data([...bottomKeys])
      .join('line')
    ...
    leftTEXT.map((texts, index) => {
      g.append('g')
        ...
        .html((d, i) => {
          if (i === 0) {
            const value = d.split(',')
            return `<tspan>${value[0]}</tspan><tspan dx="${-35 + index * 10}" dy="20">${
              value[1]
            }</tspan>`
          }
          return `${d}`
  ...
  function drwaTopData(svg) {
    ...
    // 绘制竖线和时间汉字
    const data = new Array(options.timeArr.length * 7)
      .fill('')
      .map((d, i) => options.timeArr[i % options.timeArr.length])
    g.append('g')
      .selectAll('text')
      .data(data)
      .join('text')
  ...
  function generatePointer({ pathData, type, yScaleInstance }) {
    return (event) => {
      ...
      tooltip.attr('transform', `translate(${xScale(i) + viewConfig.micoStep},${yPos})`)
