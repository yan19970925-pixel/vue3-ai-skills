...
export function getData(allData) {
  ...
  const infoData = allData.grParamBOS[0] // 头信息
  ...
  console.log(infoData.postpartum, '产后日数')
  // 写死的先
  infoData.dateClosed = {
  ...
  const earCool = getTypeAnimalHeat('003', rowsData, '4') // 耳温改成直肠体温
  ...
  const datasetPulse = getHeartRate('002', rowsData, symbolDegreesOnline, symbolTopDegreesOnline)
...
export function drawBottomMask(svg, viewConfig, bottomKeys) {
  const bottomLen = (bottomKeys || BOTTOM_KEYS).length
  console.log('bottomLen', bottomLen)
  ...
  g.append('rect')
    ...
    .attr('width', viewConfig.contentWidth)
    .attr('height', LINE_HEIGHT * (bottomLen + 5))
    .attr('stroke', viewConfig.stroke)
    .attr('fill', '#fff')
    .attr('style', 'stroke-width: 0')
...
export function drawSpecialText(svg, viewConfig, textData) {
  ...
  g.append('g')
    ...
    .attr('x', (d, i) => viewConfig.step + i * viewConfig.micoStep + textLeftMargin)
    .attr('y', () => {
